...
(function () {
  ...
  if (new URLSearchParams(location.search).get("demo") === "1") {
    ...
    document.addEventListener("DOMContentLoaded", function () {
      ...
      bar.innerHTML = '<span style="color:rgba(255,255,255,.7);">🔍 <b style="color:#c8a96e;">Demo mode</b> — fictional data, nothing is saved.</span>'
        ...
        + '<a href="/en/login?tab=register" style="background:linear-gradient(135deg,#c8a96e,#d4b97e);color:#0f1523;padding:8px 18px;border-radius:8px;font-weight:800;font-size:12px;text-decoration:none;white-space:nowrap;letter-spacing:.3px;">Start free →</a>'
  ...
  async function initAuth() {
    try {
      ...
      if (!session) {
        ...
        window.location.href = "/en/login?cleared=1";
        return;
      }
  ...
  async function handlePendingInvite() {
    ...
    if (!session) {
      ...
      const loginUrl = "/en/login?invite=" + encodeURIComponent(token);
